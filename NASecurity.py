...
class WorkerThread(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            if not required_cols.issubset(df.columns):
                ...
                self.log_manager.add_log("", "", "", f"缺少欄位: {', '.join(missing)}", is_error=True)
                return
            ...
            for i, row in df.iterrows():
                ...
                self.status_update.emit(f"處理用戶: {user} ({operation})", "black")
    ...
    def _check_user(self, user: str) -> Dict[str, Any] | None:
        ...
        try:
            ...
            self.log_manager.add_log(user, "", "", f"查詢失敗: {str(e)}", is_error=True)
            return None
...
class NASecurity(QMainWindow):
    ...
    def _setup_ui(self):
        ...
        self.ip_entry = self._add_field(input_layout, "NAS IP:", "例如: 10.57.78.62")
        self.admin_entry = self._add_field(input_layout, "管理員帳號:")
        self.pwd_entry = self._add_field(input_layout, "管理員密碼:", is_password=True)
        self.otp_entry = self._add_field(input_layout, "雙重驗證碼:", "輸入 6 位驗證碼，若無則留空")
    ...
    def _validate_inputs(self) -> bool:
        ...
        if not re.match(r"^\d+\.\d+\.\d+\.\d+$", self.ip_entry.text()):
            QMessageBox.critical(self, "錯誤", "請輸入有效NAS IP")
            return False
