...
class NASClient:
    ...
    def change_password(self, username: str, new_password: str) -> Dict[str, Any]:
        ...
        if not result.get("success", False):
            ...
            raise Exception(f"Failed to change password: {self._get_error_message(error_code)}")
        return result
    ...
    def create_user(self, username: str, password: str) -> Dict[str, Any]:
        ...
        if not result.get("success", False):
            error_code = result.get("error", {}).get("code")
            raise Exception(f"Failed to create user: {self._get_error_message(error_code)}")
        return result
